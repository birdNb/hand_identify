...
与手柄 /cmd_vel 冲突时已从 hand_follow_robot 主流程移除。
若要恢复（模块名以数字开头，需用 importlib 加载）::

    import importlib.util, os
    _p = os.path.join(os.path.dirname(__file__), "5_finger_locomotion.py")
...
然后在 hand_follow_robot.py 主循环中实例化 FingerLocomotionStack 并调用 update() / shutdown()。
...
class FingerLocomotionStack:
    """
    五指跟手栈：在 hand_follow_robot 主循环中每帧调用 update()。
    默认不在主程序中实例化。
    """
